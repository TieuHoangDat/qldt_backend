...
public class TermDto implements Comparable<TermDto>{
    ...
    private List<GroupRegistrationDto> liGR;
    private double avg_10, avg_4, tl_10, tl_4;
    private int total_credit, tl_credit, tuition;

    public TermDto(String term, List<GroupRegistrationDto> liGR) {
        ...
        for(GroupRegistrationDto x : liGR) {
            if(x.getGroup().getCourse().getNotcal() == 0) {
                sum10 += x.getGrade_10() * x.getGroup().getCourse().getNum_credit();
                sum4 += x.getGrade_4() * x.getGroup().getCourse().getNum_credit();
                tmp += x.getGroup().getCourse().getNum_credit();
            }
        }
        total_credit = tmp;
        tuition = tmp * 550000;
    ...
    public List<GroupRegistrationDto> getLi() {
        return liGR;
    }

    public void setLi(List<GroupRegistrationDto> liGR) {
        this.liGR = liGR;
    }
